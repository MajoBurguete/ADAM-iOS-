...
class StoryModelNVL1 {
    ...
    func generateParagraph() -> NSMutableAttributedString {
        ...
        let secondVariant = variants[randomStory][0][0][randomVariant2]
        if let range = paragraph.range(of:"#") {
            paragraph = paragraph.replacingCharacters(in: range, with:secondVariant)
        }
        
        let randomVariant3 = Int.random(in: 0..<3)
        let thirdVariant = variants[randomStory][0][0][randomVariant3]
        if let range = paragraph.range(of:"#") {
        ...
        finalParagraph.addAttribute(NSAttributedString.Key.foregroundColor, value: UIColor.green, range: range1)
        
        let finalParNSS = finalParagraph.string as NSString;
        
        let range2 = (finalParNSS).range(of: secondVariant)
        finalParagraph.addAttribute(NSAttributedString.Key.foregroundColor, value: UIColor.green, range: range2)
        
        let range3 = (paragraph as NSString).range(of: thirdVariant)
        finalParagraph.addAttribute(NSAttributedString.Key.foregroundColor, value: UIColor.blue, range: range3)
